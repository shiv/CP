...
template <typename T> void myshuffle(vector<T> &a) { for (int i = a.size() - 1; i > 0; i--) swap(a[i], a[rand_int(0, i)]); }
...
void solve(int tc = 0) {
    ...
    while (m--) {
        ...
        set<vii> s;
        if (A - 1 >= 0)
            s.insert({A - 1, A, y[A - 1], y[A]});
        if (A + 1 < n)
            s.insert({A, A + 1, y[A], y[A + 1]});
        if (B - 1 >= 0)
            s.insert({B - 1, B, y[B - 1], y[B]});
        if (B + 1 < n)
            s.insert({B, B + 1, y[B], y[B + 1]});

        swap(x[a], x[b]);
        swap(y[A], y[B]);

        for (auto& v : s) {
            if (v[2] > v[3] && y[v[0]] < y[v[1]])
                ans -= 1;
            if (v[2] < v[3] && y[v[0]] > y[v[1]])
                ans += 1;
        }
