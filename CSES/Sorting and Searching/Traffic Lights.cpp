...
void solve(int tc = 0) {
    ...
    for (int i = 0; i < n; i++) {
        auto it = prev(t.upper_bound({a[i], 0}));
        auto [start, len] = *it;
        s.erase({len, start});
        t.erase({start, len});
        s.emplace(a[i] - start, start);
        t.emplace(start, a[i] - start);
        s.emplace(start + len - a[i], a[i]);
        t.emplace(a[i], start + len - a[i]);
        // dbg(t);
