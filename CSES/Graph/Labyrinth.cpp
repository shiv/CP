...
void solve(int tc = 0) {
    ...
    while(!bfs.empty()) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (!(0 <= next.F && next.F < n && 0 <= next.S && next.S < m))
                continue;
            if (!visited[next.F][next.S] && (a[next.F][next.S] == '.' || a[next.F][next.S] == 'B')) {
                bfs.push(next);
                b[next.F][next.S] = c[i];
    ...
    if (!visited[B_index.F][B_index.S]) {
        out(false);
        return;
    }
