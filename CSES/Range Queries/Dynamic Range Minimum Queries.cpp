...
public:
    ...
    public:
        ...
        int val, lazy;
        // set neutral values
        node(int _val = inf, int _lazy = inf) : val(_val), lazy(_lazy) {}

        // merging of two nodes
        friend node merge(node &a, node &b) {
            node c = min(a.val, b.val);
            return c;
        }

        // lazy operation
        void apply(int lx, int rx, node other) {
            val = other.val;
            // lazy = lazy + other.lazy;
        }
    ...
    void update(int l, int r, input v, int x, int lx, int rx) {
        if (lx >= r || l >= rx)
            return;
        if (lx >= l && rx <= r) {
            nodes[x].apply(lx, rx, make_node(v));
            return;
        }
        push(x, lx, rx);
        int m = (lx + rx) >> 1;
