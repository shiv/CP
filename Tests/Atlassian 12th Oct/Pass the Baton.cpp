...
pair<int,int> solve(int n, int t) {
    int total = 2*n - 2;
    t--;
    int pos = t%total;
    if (pos < n-1) {
        return make_pair(pos+1, pos+2);
    } else {
        pos -= n-1;
        return make_pair(n - pos, n - pos - 1);
    }
}

vector<int> batonPass(int friends, long time) {
#define int int64_t
    int total = 2 * (friends - 1);
    time -= 1;
    int pos = time % total;
    if (pos < friends - 1) return {pos + 1, pos + 2};
    pos -= friends-1;
    return {friends - pos, friends - pos - 1};
#undef int
}
