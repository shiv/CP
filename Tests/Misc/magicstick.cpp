...
int magicstick(int n, vector<int> stick, vector<int> cost) {

    auto tot_cost = [&] (int x) {
        int tot_cost = 0;
        for (int i = 0; i < n; i++) {
            tot_cost += abs(stick[i] - x) * cost[i];
        }
        return tot_cost;
    };

    int low = 0, high = *max_element(stick.begin(), stick.end()) + 1;
    ...
    while (low < high) {
        ...
        int cost1 = tot_cost(mid1);
        int cost2 = tot_cost(mid2);
...
signed main() {
    ...
    for (int tt = 1; tt <= t; tt++) {
        cout << magicstick(3, {1, 2, 3}, {20, 30, 40});
    }
