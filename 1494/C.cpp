...
void solve() {
    ...
    auto go = [&](vii A, vii B) {
    	...
    	vii c(m+1);
    	for (int i = m -1; i >= 0; i--) {
    		c[i] = c[i+1] + binary_search(all(A), B[i]);
    	}
    	int ans = c[0];
    	for (int i = 0; i < m ; i++) {
    		int items = ub(all(A), B[i]) - A.begin();
    		int upto = ub(all(B), B[i] - items) - B.begin();
    		int elements = i - upto + 1;
    		amax(ans, elements + c[i+1]);
    	}
    	return ans;
    };
