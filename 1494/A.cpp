/**
 *    author:  Shivam Gupta
 *    created: 02.03.2021 23:35:49
**/

// #undef _GLIBCXX_DEBUG 1
#include <bits/stdc++.h>
using namespace std;
...
#define Fox(i,a,b) for(i=a-1;i>=b;i--)
#define Fore(i,a,b) for(i=a;i<=b;i++)
#define Forx(i,a,b) for(i=a;i>=b;i--)
...
template <typename T>
void inp(T& v, int a, int b) { FOR(i,a,b) cin>>v[i]; }
template <typename T>
void oup(T& v, int a, int b) { FOR(i,a,b) cout<<v[i]<<" "; cout<<'\n'; }
...
void myshuffle(vt<T> &a){
  FOR(i,1,a.sz) swap(a[i],a[rand_int(0,i)]);
}
...
viii readGraph(int n, int m) { viii g(n); int a, b;
    FOR(i,1,m) { cin>>a>>b; a--; b--; g[a].pb(b); g[b].pb(a); }
    return g;
}
...
void solve() {
    int i, j, k, l, n, m, q, x=0, y=0, z=0, ans=0, res=0, sum=0;
