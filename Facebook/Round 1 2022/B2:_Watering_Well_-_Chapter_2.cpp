...
void solve() {
    ...
    auto fun = [&] (vector<int> &pts, set<int> &st, map<int, Mint> &d) {
        int l = 0, prev = -1;
        int lcnt = 0, rcnt = 0;
        Mint lsum = 0, rsum, lsqsum = 0, rsqsum = 0;
        for (auto &x : pts) {
            rcnt++;
            rsum += (x - prev);
            rsqsum += (x - prev) * (x - prev);
        }
        for (auto &x : st) {
            int cnt = 0;
            Mint sum = 0, sqsum = 0;
            Mint gap = x - prev;
            while (l < n && pts[l] <= x) {
                sqsum += (x - pts[l]) * (x - pts[l]);
                sum += x - pts[l];
                cnt++;
                rsqsum -= (pts[l] - prev) * (pts[l] - prev);
                rsum -= pts[l] - prev;
                rcnt--;
                l++;
            }
            // prev ke left mein
            lsqsum += gap * gap * lcnt + 2 * gap * lsum + sqsum;
            lsum += lcnt * gap + sum;
            lcnt += cnt;
            // mere right mein
            rsqsum += gap * gap * rcnt - 2 * gap * rsum;
            rsum -= gap * rcnt;
            d[x] = lsqsum + rsqsum;
            prev = x;
        }
