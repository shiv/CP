...
void solve(int tc = 0) {
    ...
    map<long double, set<int>> m;
    vector<set<long double>> op(n);

    vector<pii> ans;

    For (i, 0, n) {
    	long double x = 1.0 * (p[i][2] * p[i][1]) / (p[i][3] * (p[i][0] + p[i][1]));
    	long double y = 1.0 * ((p[i][2] + p[i][3]) * p[i][1]) / (p[i][3] * p[i][0]);
    	m[x].in(i);
    	m[y].in(i);
    	op[i].in(x);
    	op[i].in(y);
    ...
    function<void (long double)> check;
    check = [&] (long double k) {
    	if (sz(m[k]) == 2) {
    		int a = *m[k].begin();
    		m[k].erase(m[k].begin());
