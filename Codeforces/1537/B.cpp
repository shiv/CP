...
void solve(int tc = 0) {
    ...
    auto abso = [] (int a, int b, int c, int d) {
    ...
    for (int p = 0; p < 4; p++) {
        for (int q = p + 1; q < 4; q++) {
            int res = 0;
            res += abso(v[p].first, v[p].second, v[q].first, v[q].second);
            res += abso(i, j, v[q].first, v[q].second);
            res += abso(v[p].first, v[p].second, i, j);
            if (ans < res) {
                ans = res;
                u = {v[p].first, v[p].second, v[q].first, v[q].second};
            }
        }
    }
