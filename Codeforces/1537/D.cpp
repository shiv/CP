...
bool P[N + 1] = {false};
vector<int> primes(int n) { vector<int> p; for (int j, i = 2; i <= n; i++) if (!P[i]) for (p.push_back(i), j = i * i; j <= n; j += i) P[j] = true; return p; }
