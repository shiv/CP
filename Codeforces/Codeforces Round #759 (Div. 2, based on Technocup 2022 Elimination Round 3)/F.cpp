...
void solve() {
    ...
    st.modify(mp[0], mp[a[0]], 1);

    for (int i = 1; i < n; i++) {
        // if (a[i - 1] <= a[i]) {
        //     st.modify(0, a[i - 1], a[i - 1] - 1);
        //     st.modify(a[i - 1], a[i], a[i - 1]);
        // }
        // else {
        //     st.modify(0, a[i], a[i] - 1);
        //     st.modify(a[i], a[i - 1], a[i]);
        // }
        int mx = max(a[i - 1], a[i]);
        int mn = min(a[i - 1], a[i]);
        st.modify(mp[0], mp[mn], mn - 1);
        st.modify(mp[mn], mp[mx], mn);
        doit();
    }



    Mint ans = st.get(m - 1).val;
