...
void myshuffle(vt<T> &a){
  for(int i = 0; i < a.sz; i++) swap(a[i],a[rand_int(0,i)]);
}
...
vt<vt<pii>> readGraph(int n, int m, int o) { vt<vt<pii>> g(n); int a, b, c;
    for(int i=0; i<m; i++) { cin >> a >> b >> c; a--; b--; g[a].pb({b,c}); g[b].pb({a,c}); }
    return g;
}
...
vii dijkstra(vt<vt<pii>> &g, int start) {
    int n = g.size();
    vii dis(n, inf), prev(n, -1);
    dis[start] = 0;
    set<pii> non_vis;
    non_vis.in({0,start});
    while(!non_vis.empty()) {
        int parent = non_vis.begin()->second;
        non_vis.erase(non_vis.begin());
        for(auto ele : g[parent]) {
            int child = ele.first;
            int distance = ele.second;
            if(dis[parent] + distance < dis[child]) {
                non_vis.erase({dis[child],child});
                dis[child] = dis[parent] + distance;
                prev[child] = parent;
                non_vis.in({dis[child], child});
            }
        }
    }
    return prev;
...
void solve() {
    int n, m;
    cin >> n >> m;
    vt<vt<pii>> g = readGraph(n, m, 0);
    vii prev = dijkstra(g, 0);
    vii path, vis(n);
