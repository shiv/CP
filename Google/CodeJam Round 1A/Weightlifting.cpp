...
void solve() {
    ...
    function<void (int, int, stack<int>, vector<int>)> go = [&] (int i, int op, stack<int> st, vector<int> last_freq) {
        if (i == e) {
            amin(ans, op + st.size());
            return;
        }
        ...
        vector<int> p;
        for (int j = 0; j < w; j++) {
            for (int k = 0; k < curr_freq[j] - last_freq[j]; k++) {
                p.push_back(j);
            }
        }
        // dbg(i, curr_freq, last_freq);
        ...
        do {
            ...
            go(i + 1, op + p.size(), tt, curr_freq);
        } while (next_permutation(p.begin(), p.end()));

    };

    go(0, 0, stack<int>(), vector<int>(w, 0));
