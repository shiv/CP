...
int P[N + 1];
void primes(int n = N) { for (int i = 2, j; i <= n; i++) if (!P[i]) for (P[i] = i, j = i * i; j <= n; j += i) P[j] = i; }
...
void sieve() {
	for (int i = 2; i < N; i++) {
		...
		int prod;
		for (int j = 0; j < (int) prims.size() && prims[j] <= spf[i] && (prod = prims[j] * i) < N; j++) {
			spf[prod] = prims[j];
			prime[prod] = false;
		}
	}
